...
Servidor legado (debug): `node test-server.js` (porta 8000).
...
O arquivo `playwright.storybook.config.ts` roda apenas o smoke `storybook-hub.smoke.spec.ts`. Garanta que o hub esteja rodando (`npm run dev --workspace @adsmagic/storybook-hub`) ou exporte `STORYBOOK_URL` apontando para o build estatico.
